class ProductDao:
    ...
    def get_product(self, product_id, session):
        ...
        color_list = [{"id": color.id, "color": color.color_name} for color in colors]
        product_info["colors"] = color_list

        size_list = [{"id": size.id, "size": size.size_name} for size in sizes]
        product_info["sizes"] = size_list

        image_list = [{"id": image.id, "image_url": image.URL} for image in images]
